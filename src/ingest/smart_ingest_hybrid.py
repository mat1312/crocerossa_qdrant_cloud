"""
Smart Ingest Pipeline v3.2 - HYBRID SEARCH EDITION (Qdrant 1.16.2 Compatible)
...
CHANGELOG v3.2:
- NEW: Text index con STEMMER ITALIANO (Snowball)
- NEW: Stopwords italiane rimosse automaticamente
- NEW: ASCII folding (perché → perche)
- NEW: Phrase matching per ricerche esatte con "virgolette"
- FIX: Payload indexes per Qdrant 1.16+ (richiesti per filtri)

CHANGELOG v3.1:
- FIX: Aggiunto creazione esplicita degli indici payload richiesti da Qdrant 1.16+

Features:
- Dense vectors (OpenAI text-embedding-3-large) per ricerca semantica
- Sparse vectors (BM25 via FastEmbed) per ricerca keyword
- TEXT INDEX italiano con stemming per matching flessibile
...
Usage:
    python ingest1162.py new_docs/              # Processa cartella
    python ingest1162.py doc.pdf                # Singolo file
    python ingest1162.py new_docs/ --dry-run    # Preview
    python ingest1162.py new_docs/ --no-sparse  # Solo dense
"""
...
from qdrant_client.models import (
    ...
    SparseVectorParams,
    StopwordsSet,
    # Text Index con Stemming (Qdrant 1.15+)
    TextIndexParams,
    TextIndexType,
    TokenizerType,
    VectorParams,
...
class DocumentParser:
    ...
    def parse_file(self, filepath: str) -> str:
        ...
        logger.info(f"📄 Parsing ({mode_str}): {filepath.name}")
...
class PageAwareChunker:
    ...
    def chunk_document(self, content: str, filename: str, source_path: str, file_hash: str) -> list[Document]:
        ...
        logger.info(f"✂️  Chunking: {filename} → {len(optimized_chunks)} chunks (Page-Aware + Optimized)")
    ...
    def _optimize_chunks(self, chunks: list[Document], min_chars: int = 400) -> list[Document]:
        ...
        for doc in chunks:
            ...
            if buffer_doc:
                ...
                # Se ora è abbastanza grande, flush
...
# ============================================================================
# QDRANT HYBRID MANAGER - UPDATED FOR QDRANT 1.16.2
# ============================================================================
...
class QdrantHybridManager:
    ...
    QDRANT 1.16+ FIX:
    - Crea indici espliciti per i campi usati nei filtri
    """
    ...
    def ensure_collection_exists(self) -> bool:
        ...
        QDRANT 1.16+ FIX: Crea anche gli indici sui campi del payload.
        """
        try:
            ...
            if self.config.qdrant_collection not in collection_names:
                ...
                logger.info("✅ Collection creata con schema:")
                ...
                # === QDRANT 1.16+ FIX: Crea indici payload ===
                self._create_payload_indexes()

            else:
                logger.info(f"✅ Collection esistente: {self.config.qdrant_collection}")
                # Verifica e crea indici se mancanti (per collection esistenti)
                self._create_payload_indexes()
    ...
    def _create_payload_indexes(self):
        """
        QDRANT 1.16+ FIX:
        Crea indici espliciti sui campi del payload usati nei filtri.
        Include TEXT INDEX con STEMMER ITALIANO per full-text search avanzato.
        """
        logger.info("📑 Creazione indici payload per Qdrant 1.16+...")

        # Lista dei campi KEYWORD/INTEGER da indicizzare
        simple_indexes = [
            # Campi nested in metadata
        ...
        for field_name, field_type in simple_indexes:
            try:
                ...
                logger.info(f"   ✅ Indice creato: {field_name} ({field_type})")
            except Exception as e:
                if "already exists" in str(e).lower():
                    logger.info(f"   ℹ️  Indice esistente: {field_name}")
                else:
                    logger.warning(f"   ⚠️  Impossibile creare indice {field_name}: {e!s}")

        # === TEXT INDEX CON STEMMER ITALIANO ===
        # Abilita ricerca fuzzy: "corsi" trova "corso", "volontari" trova "volontario"
        ...
        try:
            self.client.create_payload_index(
                ...
                field_schema=TextIndexParams(
                    ...
                    # Tokenizer multilingue (supporta Unicode)
                    tokenizer=TokenizerType.MULTILINGUAL,
                    # Filtro lunghezza token
                    min_token_len=2,  # Ignora "a", "e", "o"
                    max_token_len=40,  # Ignora parole troppo lunghe
                    # Normalizzazione testo
                    lowercase=True,  # "CRI" = "cri"
                    ascii_folding=True,  # "perché" = "perche"
                    # STEMMER ITALIANO (Snowball)
                    stemmer=SnowballParams(type="snowball", language=SnowballLanguage.ITALIAN),
                    # Stopwords italiane
                    stopwords=StopwordsSet(languages=[Language.ITALIAN]),
                    # Abilita ricerca frasi esatte con "virgolette"
                    phrase_matching=True,
                ),
                wait=True,
            )
            logger.info("   ✅ TEXT INDEX italiano configurato:")
            logger.info("      - Stemmer: Snowball Italian")
            logger.info("      - Stopwords: Italian")
            logger.info("      - ASCII folding: abilitato")
            logger.info("      - Phrase matching: abilitato")
        except Exception as e:
            if "already exists" in str(e).lower():
                logger.info("   ℹ️  TEXT INDEX già esistente su page_content")
            else:
                logger.warning(f"   ⚠️  Impossibile creare TEXT INDEX: {e!s}")

        logger.info("📑 Indici payload configurati.")
    ...
    def count_chunks_by_filename(self, filename: str) -> int:
        ...
        try:
            ...
            logger.error(f"❌ Errore conteggio: {e!s}")
            return 0
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"❌ Errore stats: {e!s}")
            return {}
...
class HybridIngestPipeline:
    ...
    def process_file(self, filepath: str, mode: str = "replace", dry_run: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            chunks = self.chunker.chunk_document(
                content=content, filename=filename, source_path=str(filepath.absolute()), file_hash=file_hash
            )
    ...
    def process_folder(self, folder_path: str, mode: str = "replace", dry_run: bool = False) -> list[dict[str, Any]]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Smart Ingest Pipeline v3.1 - HYBRID SEARCH (Qdrant 1.16.2 Compatible)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Esempi:
  python ingest1162.py docs/                    # Processa cartella
  python ingest1162.py documento.pdf            # Singolo file
  python ingest1162.py docs/ --dry-run          # Preview
  python ingest1162.py docs/ --no-sparse        # Solo semantic
  python ingest1162.py docs/ --chunk-size 512   # Chunks più piccoli
        """,
    )

    parser.add_argument("input_path", help="File o cartella da processare")
    parser.add_argument(
        "--mode", choices=["replace", "add-only"], default="replace", help="Modalità: replace (default) o add-only"
    )
    parser.add_argument("--dry-run", action="store_true", help="Mostra cosa farebbe senza eseguire")
    parser.add_argument("--no-sparse", action="store_true", help="Disabilita sparse vectors (solo dense)")
    parser.add_argument(
        "--premium", action="store_true", help="Forza Premium Mode per TUTTI i file (Excel è già attivo di default)."
    )
    parser.add_argument(
        "--chunk-size", type=int, default=2000, help="Dimensione chunks (default: 2000 chars ~500 tokens)"
    )
    parser.add_argument("--chunk-overlap", type=int, default=300, help="Overlap chunks (default: 300 = 15%%)")
    ...
    # Header
    print("\n" + "=" * 60)
    print("🚀 SMART INGEST PIPELINE v3.1 - HYBRID SEARCH (Qdrant 1.16.2)")
    print("=" * 60)
    ...
    start_time = time.time()
    results = pipeline.run(input_path=args.input_path, mode=args.mode, dry_run=args.dry_run)
    elapsed = time.time() - start_time
